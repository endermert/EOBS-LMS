<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="RPR_AraKarne1.aspx.cs" Inherits="EOBS_001.RPR_AraKarne1" %>
...
<asp:Content ID="ContentRPRHaftalikKarne" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <asp:Panel runat="server" ID="Panel2">
     <div class="main_container container_16 clearfix full_size">
		<div class="box grid_16">
			<h2 class="box_head">OGRENCi RAPORU PARAMETRELERi<span></span></h2>
			<div class="controls">
    ...
    <div class="main_container container_16 clearfix full_size">
		<div class="box grid_16">
			<h2 class="box_head">OGRENCi RAPORU<span></span></h2>
			<div class="controls">
				<a href="#" class="grabber"></a>
				<a href="#" class="toggle"></a>
			</div>
            ...
            <LocalReport ReportPath="RPR_AraKarne1.rdlc"  >
...
WHERE        (DAMLACIKLAR.OKL_ID = @OKL_ID) 
...
WHERE        (OZELNOT.OZN_AKTIF = 1) AND (OZELNOTLAR.OKL_ID = @OKL_ID)
ORDER BY OZELNOT.OZN_TAR DESC">
         ...
         <asp:SqlDataSource ID="SDS_OdevSnc" runat="server" ConnectionString="<%$ ConnectionStrings:EOBS_001ConnectionString %>" SelectCommand="SELECT ODEVSNC.KUL_ID, ODEVSNC.ODE_NOT, KULLANICILAR.KUL_AD, ODEVSNC.ODE_SNC FROM ODEVSNC INNER JOIN ODEVSNCLAR ON ODEVSNC.SNC_ID = ODEVSNCLAR.SNC_ID INNER JOIN KULLANICILAR ON ODEVSNCLAR.KUL_ID = KULLANICILAR.KUL_ID WHERE (ODEVSNC.ODE_AKTIF = 1) AND (ODEVSNCLAR.OKL_ID = @OKL_ID) ORDER BY ODEVSNC.ODE_TAR DESC">
            <SelectParameters>
                <asp:Parameter DefaultValue="2" Name="HFT_ID" />
                <asp:Parameter DefaultValue="3" Name="OKL_ID" />
            </SelectParameters>
        </asp:SqlDataSource>
        <asp:SqlDataSource ID="SDS_Sinavlar" runat="server" ConnectionString="<%$ ConnectionStrings:EOBS_001ConnectionString %>" SelectCommand="SELECT        SINAVLAR.SNV_ID, SINAVDETAY.STP_ID, SINAVLAR.SNV_AD, DRSSNVSNC.KUL_ID, DERSLER.DRS_AD, DRSSNVSNC.DSN_SS, DRSSNVSNC.DSN_DS, 
                         DRSSNVSNC.DSN_YS, DRSSNVSNC.DSN_NS, DRSSNVSNC.DSN_SRASUB, DRSSNVSNC.DSN_SRASNF, DRSSNVSNC.DSN_SRAOKL, 
                         DRSSNVSNC.DSN_BSRYZD, DRSSNVSNC.DSN_BSRYRM, SINAVSNC.SNS_SS, SINAVSNC.SNS_DS, SINAVSNC.SNS_YS, SINAVSNC.SNS_NS, 
                         SINAVSNC.SNS_SRASUB, SINAVSNC.SNS_SRASNF, SINAVSNC.SNS_SRAOKL, SINAVSNC.SNS_BSRYZD, SINAVSNC.SNS_BSRYRM, 
                         SINAVSNC.SNS_PUAN
FROM            SINAVLAR INNER JOIN
                         DRSSNVSNC ON SINAVLAR.SNV_ID = DRSSNVSNC.SNV_ID INNER JOIN
                         DERSLER ON DRSSNVSNC.DRS_ID = DERSLER.DRS_ID INNER JOIN
                         SINAVSNC ON DRSSNVSNC.KUL_ID = SINAVSNC.KUL_ID AND DRSSNVSNC.SNV_ID = SINAVSNC.SNV_ID INNER JOIN
                         SINAVDETAY ON SINAVLAR.SDT_ID = SINAVDETAY.SDT_ID
WHERE        (SINAVLAR.SNV_DEG = 1) AND (SINAVLAR.OKL_ID = @OKL_ID) AND (SINAVLAR.SNV_KARNE = 1)">
            <SelectParameters>
                <asp:Parameter DefaultValue="3" Name="OKL_ID" />
            </SelectParameters>
        </asp:SqlDataSource>
        <asp:SqlDataSource ID="SDS_DersNot" runat="server" ConnectionString="<%$ ConnectionStrings:EOBS_001ConnectionString %>" SelectCommand="SELECT KARNEDERSNOT.KDN_ID, DERSLER.DRS_AD, KARNEDERSNOT.KUL_ID, KARNEDERSNOT.KDN_S1, KARNEDERSNOT.KDN_S2, KARNEDERSNOT.KDN_S3, KARNEDERSNOT.KDN_S4, KARNEDERSNOT.KDN_PROORT, KARNEDERSNOT.KDN_PERORT, KARNEDERSNOT.KDN_NOT, KARNEDERSNOT.KDN_PUAN FROM KARNEDERSNOT INNER JOIN DERSLER ON KARNEDERSNOT.DRS_ID = DERSLER.DRS_ID INNER JOIN KULLANICILAR ON KARNEDERSNOT.KUL_ID = KULLANICILAR.KUL_ID WHERE (KULLANICILAR.OKL_ID = @OKL_ID)">
         ...
         <asp:SqlDataSource ID="SDS_Devamsiz" runat="server" ConnectionString="<%$ ConnectionStrings:EOBS_001ConnectionString %>" SelectCommand="SELECT KUL_ID, DVM_AD, DVM_GUN, DVM_MAZERET, DVM_TAR FROM DEVAMSIZ WHERE (OKL_ID = @OKL_ID)">
            <SelectParameters>
                <asp:Parameter DefaultValue="3" Name="OKL_ID" />
            </SelectParameters>
        </asp:SqlDataSource>
        <asp:SqlDataSource ID="SDS_Davranis" runat="server" ConnectionString="<%$ ConnectionStrings:EOBS_001ConnectionString %>" SelectCommand="SELECT DAVRANIS.KUL_ID, DAVRANIS.DAV_AD, DAVRANIS.DAV_TAR, KULLANICILAR.KUL_AD FROM DAVRANIS INNER JOIN KULLANICILAR ON DAVRANIS.DAV_KULID = KULLANICILAR.KUL_ID WHERE (DAVRANIS.OKL_ID = @OKL_ID) ORDER BY DAVRANIS.DAV_TAR DESC">
            <SelectParameters>
                <asp:Parameter DefaultValue="3" Name="OKL_ID" />
            </SelectParameters>
        </asp:SqlDataSource>
        <asp:SqlDataSource ID="SDS_HepTablo" runat="server" ConnectionString="<%$ ConnectionStrings:EOBS_001ConnectionString %>" SelectCommand="SELECT HEP_ID, HEP_GRUP, KUL_ID, HEP_H1, HEP_G1, HEP_H2, HEP_G2, HEP_H3, HEP_G3, HEP_H4, HEP_G4 FROM HEPTABLO WHERE (OKL_ID = @OKL_ID)">
            <SelectParameters>
                <asp:Parameter DefaultValue="3" Name="OKL_ID" />
            </SelectParameters>
        </asp:SqlDataSource>
